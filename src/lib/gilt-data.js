...
export class GiltDataFetcher {
  ...
  async getFallbackData() {
    try {
      ...
      // Import the complete 37-gilt dataset directly  
      const { GiltDataFetcher: CompleteGiltDataFetcher } = await import('./gilt-data-complete.js');
      const completeDataFetcher = new CompleteGiltDataFetcher();
      
      // The parseMaturityDate function actually contains the comprehensive gilt array
      const completeGiltData = completeDataFetcher.parseMaturityDate(''); // This returns the comprehensive gilt array
      ...
      // Process the data through the same pipeline as other data
